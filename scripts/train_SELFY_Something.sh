...
python3 -u main.py ${dataset_name} RGB ${train_path} ${val_path} --arch ${netType} --num_segments ${num_segments} --mode ${mode} --gd 20 --lr ${learning_rate} --lr_steps 30 40 --epochs 50 -b ${batch_size} -i ${iter_size} -j ${num_workers} --dropout ${dropout} --snapshot_pref ${mainFolder}/${subFolder}/${snap_pref} --consensus_type avg --eval-freq 1 --rgb_prefix img_ --pretrained_parts finetune --no_partialbn --val_output_folder ${mainFolder}/${subFolder}/validation -p 20 --nesterov "True" --resume ${mainFolder}/${lastCheckpoint} 2>&1 | tee -a ${mainFolder}/${subFolder}/training/log.txt    
